...
def plot_generations_single(csv_files, column_name, phase, task_set, pdf = None):
    """
    ...
    for idx, csv_file in enumerate(csv_files):
        try:
            ...
            if column_name not in df.columns:
                ...
                continue
            # filter df based on phase
            if "phase" in df.columns:
                df = df[df["phase"] == phase]
            # filter df based on task_set
            if "task_set" in df.columns:
    ...
    plt.ylabel(capitalize_snake_case(column_name), fontsize=24)
...
def plot_generations_multiple(csv_files, column_names, phase, task_set):
    ...
    output_filename = plot_dir + "/" + get_unique_filename(f"{prefix}_all_vs_generations_p{phase}{task_set}.pdf")

    with PdfPages(output_filename) as pdf:
        for column_name in column_names:
            plot_generations_single(csv_files, column_name, phase, task_set, pdf)
...
if __name__ == "__main__":
    ...
    parser.add_argument('task_set', type=str, default="_0_", help='Task set')
    ...
    try:
        if column_name == "all":
            ...
            plot_generations_multiple(valid_files, column_names, args.phase, args.task_set)
        else:
            plot_generations_single(valid_files, column_name, args.phase, args.task_set)
    except Exception as e:
        print(f"Error: {str(e)}")
        sys.exit(1)
