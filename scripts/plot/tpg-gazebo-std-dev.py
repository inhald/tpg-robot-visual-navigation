...
# Global plotting settings for paper legibility
plt.rcParams.update({
    "font.size": 13,
    "axes.titlesize": 15,
    "axes.labelsize": 14,
...
def load_seed_curves_from_experiment(exp_dir: str):
...
def aggregate_group(experiment_dirs):
    ...
    for exp_dir in experiment_dirs:
        ...
        seed_curves = load_seed_curves_from_experiment(selection_dir)
...
if __name__ == "__main__":
    ...
    if len(gens_s) == 0 or len(gens_f) == 0:
        raise RuntimeError(
            f"Missing data. stateless curves={n_s}, stateful curves={n_f}"
        )

    L = min(len(gens_s), len(gens_f))
    gens = np.arange(L)
    mean_s, std_s = mean_s[:L], std_s[:L]
    mean_f, std_f = mean_f[:L], std_f[:L]
    ...
    plot_mean_with_std(ax, gens, mean_f, std_f, "Stateful",  COLORS["stateful"],  alpha=0.18)
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(out_dir, "stateful_vs_stateless_mean_std.pdf"),
        bbox_inches="tight"
    )
    plt.savefig(
        os.path.join(out_dir, "stateful_vs_stateless_mean_std.png"),
        dpi=600,
        bbox_inches="tight"
    )
    plt.show()
