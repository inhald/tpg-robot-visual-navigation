...
def evolve(ctx: click.Context, env: str, processes: int, seed: int, num_experiments: int):
    ...
    env_dir = helpers.create_environment_directories(TPG, env, hyper_parameters[env])
...
@click.argument("csv_files", required=False, default="all-selection")
@click.argument("column_name", required=True, default="all")
@click.option("--phase", required=False, default=0)
@click.option("--task_set", required=False, default="_0_")
@click.pass_context
def plot(ctx: click.Context, env: str, csv_files: str, column_name: str, phase: int, task_set: str):
    ...
    cmd = ["python3", plot_script_path, csv_files, column_name, str(phase), task_set]
...
@click.argument("env")
@click.option("--animate", is_flag=True, default=False, help="Enable animation.")
@click.option("-s", "--seed", help="Random seed")
@click.option("--seed-aux", help="Auxillary seed", default=42)
@click.option("--phase", default=0)
@click.option("--task_set", default="_0_")
@click.option("-t", "--task_to_replay", help="Task to replay for multitask", default=0)
@click.pass_context
def replay(ctx: click.Context, env: str, seed: int, seed_aux: int, phase: int, task_set: str, task_to_replay: int, animate: bool):
    ...
    for i, csv_file in enumerate(csv_files, 1):
        ...
        metrics = helpers.get_metrics_from_csv(csv_file, phase, task_set, task_to_replay)
        ...
        click.echo(f"Fitness: {metrics['fitness_task_'+str(task_to_replay)]}, Generation: {metrics['generation']}, Team ID: {metrics['team_id']}")
