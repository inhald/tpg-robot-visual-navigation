...
def create_hyper_param_mapping(tpg_env_var):
    ...
    # Add all parameter files in configs directory
    ...
    # Add parameter files from all experiment directories
    for entry in os.listdir(os.path.join(tpg_env_var, "experiments")):
        filename = entry + ".yaml"
        mapping[entry] = os.path.join(tpg_env_var,"experiments",entry,filename)   
