...
def get_metrics_from_csv(csv_file, phase, task_set, task_to_replay):
    # Read the CSV file
    ...
    max_fitness = df["fitness_task_" + str(task_to_replay)].max()

    # Filter the rows with that best_fitness
    # filtered = df[df["fitness_task_" + task_set] == max_fitness]
    ...
    return result_row[["fitness_task_" + str(task_to_replay), "generation", "team_id"]].to_dict()   
...
def create_environment_directories(TPG: str, env: str, parameters_file: str):
    ...
    shutil.copy(parameters_file, env_dir)            
    return env_dir
