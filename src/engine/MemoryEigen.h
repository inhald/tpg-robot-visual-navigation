...
class MemoryEigen {
  public:
   ...
   inline void MutateConstants(std::mt19937& rng, int mutation_type = 0) {
      if (mutation_type == 0) {
         ...
         auto dis1 = std::normal_distribution<double>(0.0, 0.1);
      ...
      } else if (mutation_type == 3) {  // MAPLE
         auto dis1 = std::uniform_real_distribution<double>(1.0, 2.0);
         auto dis2 = std::uniform_real_distribution<double>(0.0, 1.0);
         for (auto& m : const_memory_) {
            auto r = m.reshaped();
            for (auto& x : r) {
         ...
         auto dis1 = std::normal_distribution<double>(0.0, 0.1);
         auto dis2 = std::uniform_real_distribution<double>(0.0, 1.0);
         auto dis3 = std::uniform_real_distribution<double>(-1.0, 1.0);
         for (auto& m : const_memory_) {
            auto r = m.reshaped();
            for (auto& x : r) {
               if (dis2(rng) <= 0.05) {
         ...
         auto dis1 = std::normal_distribution<double>(0.0, 0.5);
         auto dis2 = std::uniform_real_distribution<double>(0.0, 1.0);
         for (auto& m : const_memory_) {
            auto r = m.reshaped();
            for (auto& x : r) {
