...
void instruction::SetupOps() {
  ...
  op_signatures_[COPY_SCALARS_TO_VECTOR_OP_] = {
      MemoryEigen::kVectorType_, MemoryEigen::kScalarType_};
  op_functions_[COPY_SCALARS_TO_VECTOR_OP_] =
      (&instruction::ExecuteCopyScalarsToVectorOp);
  op_names_[COPY_SCALARS_TO_VECTOR_OP_] = "COPY_SCALARS_TO_VECTOR_OP";
...
void instruction::Mutate(bool randomize, vector<bool>& legal_ops,
   ...
   if (op_ == SCALAR_CONST_SET_OP_ || op_ == VECTOR_CONST_SET_OP_ ||
       op_ == MATRIX_CONST_SET_OP_ || op_ == COPY_SCALARS_TO_VECTOR_OP_) {
      in1Src_ = 0;
   }
}
