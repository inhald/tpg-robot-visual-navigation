...
class TPG {
   public:
    ...
    void LogTeamInfo(team*, std::string);
    ...
    void FindSingleTaskFitnessRange(vector<double> &mins,
                                    vector<double> &maxs);
    vector<team *> NormalizeScoresAndRankTeams(vector<int> &set,
        vector<double> &min_scores, vector<double> &max_scores);
    void FindMultiTaskElites(vector<double> &min_scores,
    ...
    vector<size_t> num_elite_teams_;
