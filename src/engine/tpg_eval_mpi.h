...
/******************************************************************************/
inline void MaybeAddTeamToEvalSet(TPG& tpg, TaskEnv* task, team* tm,
                                  std::vector<team*>& teams_to_eval) {
   if (tpg.GetState("phase") == _TRAIN_PHASE) {  // train phase
      if (tm->numOutcomes(tpg.GetState("phase"), tpg.GetState("task")) <
          tpg.GetParam<int>("max_n_eval_train")) {
         tm->_n_eval = task->GetNumEval(tpg.GetState("phase"));
      } else {
         tm->_n_eval = 0;
      }
   } else {  // validation and test phases
      tm->_n_eval =
          task->GetNumEval(tpg.GetState("phase")) -
          tm->numOutcomes(tpg.GetState("phase"), tpg.GetState("task"));
   }
   if (tm->_n_eval > 0) {
...
inline vector<team*> GetTeamsToEval(TPG& tpg, TaskEnv* task) {
   ...
   if (tpg.GetState("phase") != _TEST_PHASE) {
      for (auto tm : root_teams) {
         MaybeAddTeamToEvalSet(tpg, task, tm, teams_to_eval);
      }
   } else {
      // Test the single training champion for each subset.
      auto PS = PowerSet(tpg.GetState("n_task"));
      for (auto& set : PS) {
         auto tm =
             tpg._eliteTeamPS[VectorToStringNoSpace(set)]
                             [tpg.GetParam<int>("fit_mode")][_TRAIN_PHASE];
         MaybeAddTeamToEvalSet(tpg, task, tm, teams_to_eval);
      }
   }
...
inline void evaluator(TPG &tpg, mpi::communicator &world, vector<TaskEnv *> &tasks) {
  ...
  evaluator_map["Gazebo"] = &EvalGazebo;
  ...
  eval_data.skip_introns = tpg.GetParam<int>("skip_introns");
      ...
      for (auto tm : eval_data.teams) {
         ...
         for (eval_data.episode = 0; eval_data.episode < eval_data.tm->_n_eval;
            ...
            if (tpg.GetParam<int>("seed_with_episode_number")) {
               // Ensure unique seeds for training, validation, test
               tpg.rngs_[AUX_SEED].seed((tpg.GetState("phase") * 1000) +
                                        eval_data.episode);
            }
...
inline void replayer(TPG& tpg, vector<TaskEnv*>& tasks) {
   ...
   eval_data.eval_result = "";
   for (auto tm : eval_data.teams) {
      if (tm->id_ != tpg.GetParam<int>("id_to_replay"))
         continue;
      eval_data.tm = tm;
      ...
      cout << "team_id,n_outcomes,mean_outcome,median_outcome" << endl;
      // cout << "mean_outcome" << endl;
      std::vector<double> outcomes;
      tpg.state_["active_task"] = tpg.GetParam<int>("task_to_replay");
      eval_data.task = tasks[tpg.GetState("active_task")];
      ...
      for (eval_data.episode = 0; eval_data.episode < eval_data.tm->_n_eval;
         ...
         if (eval_data.task->eval_type_ == "RecursiveForecast") {
            ...
            EvalControlViz(tpg, eval_data, teamUseMapPerTask,
                           ...
                           steps_per_task[tpg.GetState("active_task")]);
         } else if (eval_data.task->eval_type_ == "Gazebo") {
	   EvalGazebo(tpg, eval_data);
	 }
	 else {
            EvalMuJoco(tpg, eval_data);
         }
         tpg.FinalizeStepData(eval_data);
         outcomes.push_back(eval_data.stats_double[REWARD2_IDX]);
      }
      // tpg.printGraphDotGPTPXXI(eval_data.tm->id_, teams_visitedAllTasks,
      //                          teamUseMapPerTask, steps_per_task);

      cout << eval_data.tm->id_ << "," << outcomes.size() << ",";
      cout << VectorMean(outcomes) << "," << VectorMedian(outcomes) << endl;
      ...
      if (headless && frame_idx > 0) {
         ...
         string video_filename =
             "videos/video_" + to_string(eval_data.tpg_seed) + ".mp4";

         // Create video from frames using ffmpeg
         string ffmpeg_cmd =
             "ffmpeg -y -framerate 30 -i frames/frame_%05d.ppm -c:v libx264 "
             "-pix_fmt yuv420p " +
             video_filename;
