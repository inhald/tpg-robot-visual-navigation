...
void TPG::ProgramMutator_ActionPointer(RegisterMachine* prog_to_mu,
   ...
   if (real_dist_(rngs_[TPG_SEED]) < GetParam<double>("pmn")) {
      ...
      } else if (prog_to_mu->action_ >= 0) {
         // Changing team pointer to team pointer, just do it
         MutateActionToTeam(prog_to_mu, new_team, n_new_teams);
      } else if (new_team->n_atomic_ > 1) {
         // Changing atomic to team pointer, check graph size constraint
         // TODO(skelly): no working yet
         // set<RegisterMachine*, RegisterMachineIdComp> programs;
         // set<team*, teamIdComp> teams;
         // new_team->GetAllNodes(team_map_, teams, programs);
         // if (teams.size() < GetParam<int>("max_teams_per_graph")) {
         MutateActionToTeam(prog_to_mu, new_team, n_new_teams);
         // }
      }
   }
...
void TPG::GenerateNewTeams() {
   ...
   while (new_teams_count < GetParam<int>("n_root_gen")) {   
      uniform_int_distribution<int> disP(0, candidate_parent_teams.size() - 1);
         ...
         AddTeam(child_team);
         new_teams_count++;
         // Remove team if it violates the graph size constraint
         // TODO(skelly): this is not efficient
          set<RegisterMachine*, RegisterMachineIdComp> programs;
         set<team*, teamIdComp> teams;
         child_team->GetAllNodes(team_map_, teams, programs);
         if (teams.size() > (size_t)GetParam<int>("max_teams_per_graph")) {
            RemoveTeam(child_team);
            new_teams_count--;
         }  
   }
   ...
   builder.with_generation(GetState("t_current"))
       ...
       .with_num_elite_teams(num_elite_teams_[GetState("phase")])
   ...
   if (real_dist_(rngs_[TPG_SEED]) < GetParam<double>("p_clone_program")) {
      ...
      for (auto prog : new_team_programs) {
         if (GetParam<int>("max_team_size") == 1 ||  //Always clone in this mode
            ...
            //    set<RegisterMachine*, RegisterMachineIdComp> programs;
            //    set<team*, teamIdComp> teams;
            //    team_to_modify->GetAllNodes(team_map_, teams, programs);
            //    if (teams.size() < GetParam<int>("max_teams_per_graph")) {
...
/******************************************************************************/
// Finds the min and max evaluation outcome for each task
void TPG::FindSingleTaskFitnessRange(vector<double>& mins,
                                     vector<double>& maxs) {
   for (int task = 0; task < GetState("n_task"); task++) {
      vector<team*> teams;
      for (auto tm : GetRootTeamsInVec()) {
         if (tm->numOutcomes(GetState("phase"), task) == 0) continue;
         tm->fit_ =
             tm->GetMeanOutcome(GetState("phase"), task, GetState("fitMode"));
         teams.push_back(tm);
         // TODO(skelly): why this?
         if (GetState("phase") == _TEST_PHASE) {
            UpdateTeamPhyloData(tm);
         }
      }
      sort(teams.begin(), teams.end(), teamFitnessLexicalCompare());
      maxs[task] = (*(teams.begin()))->fit_;
      mins[task] = (*(teams.rbegin()))->fit_;
   }
}

/******************************************************************************/
// Normalize each root team's evaluation outcome in each task relative to 
// all other root teams in the same task.
// Next, set each root's fitness as it's lowest normed score in any task. 
vector<team*> TPG::NormalizeScoresAndRankTeams(vector<int>& set,
                                               vector<double>& min_scores,
                                               vector<double>& max_scores) {
   vector<team*> teams;
   for (auto tm : GetRootTeamsInVec()) {
      if (GetState("phase") == _TEST_PHASE &&
          tm->id_ != (_eliteTeamPS[VectorToStringNoSpace(set)][GetParam<int>(
                          "fit_mode")][_TRAIN_PHASE])
                         ->id_) {
         continue;
      }
      vector<double> norm_scores;
      for (size_t task = 0; task < set.size(); task++) {
         auto raw_mean_score = tm->GetMeanOutcome(GetState("phase"), set[task],
                                                  GetState("fitMode"));                                       
         // Guards for same min and max
         if (!isEqual(min_scores[set[task]], max_scores[set[task]])) {
            norm_scores.push_back(
                (raw_mean_score - min_scores[set[task]]) /
                (max_scores[set[task]] - min_scores[set[task]]));
         } else {
            norm_scores.push_back(raw_mean_score / max_scores[set[task]]);
         }
      }

      if (norm_scores.size() != set.size()) {
         die(__FILE__, __FUNCTION__, __LINE__,
             "This team should have a score for all tasks.");
      }
      tm->fit_ = *min_element(norm_scores.begin(), norm_scores.end());
...
/******************************************************************************/
// For each set in the task power set, 
// mark some teams as elite for this specific task.
void TPG::FindMultiTaskElites(vector<double>& min_scores,
                              vector<double>& max_scores) {
   auto task_power_set = PowerSet(GetState("n_task"));
   auto n_elite_per_set = GetParam<int>("n_root") / task_power_set.size();

   // First mark all teams not elite
   for (auto tm : GetRootTeamsInVec()) {
      tm->elite(GetState("phase"), false);
   }

   // Mark n_elite_per_set without replacement
   for (auto& set : task_power_set) {
      auto teams = NormalizeScoresAndRankTeams(set, min_scores, max_scores);
      sort(teams.begin(), teams.end(), teamFitnessLexicalCompare());
      size_t elite_count = 0;
      for (auto tm : teams) {
         tm->elite(GetState("phase"), true);
         num_elite_teams_[GetState("phase")]++;
         elite_count++;
         if (GetState("phase") == _TRAIN_PHASE) {
            ...
            for (int task = 0; task < GetState("n_task"); task++) {
               phylo_graph_[tm->id_].taskFitnesses.push_back(tm->GetMeanOutcome(
                   GetState("phase"), task, GetState("fitMode")));
            }
         }
         if (elite_count >= n_elite_per_set) {
            break;
         }
      }
      // Always keep track of single best elite team for each task set
      _eliteTeamPS[VectorToStringNoSpace(set)][GetState("fitMode")]
                  [GetState("phase")] = *(teams.begin());
   }
}

/******************************************************************************/
// Mark some root teams (agents) as elite to protect them from deletion
void TPG::SetEliteTeams(vector<TaskEnv*>& tasks) {
   num_elite_teams_[GetState("phase")] = 0;
   std::vector<double> min_scores(GetState("n_task"));
   std::vector<double> max_scores(GetState("n_task"));
   FindSingleTaskFitnessRange(min_scores, max_scores);
   FindMultiTaskElites(min_scores, max_scores);

   // log stats and maybe write checkpoints
   auto task_power_set = PowerSet(GetState("n_task"));
   for (auto set : task_power_set) {
      if (set.size() == 1 || set.size() == (size_t)GetState("n_task")) {
         auto tm = _eliteTeamPS[VectorToStringNoSpace(set)][GetState("fitMode")]
                               [GetState("phase")];
         LogTeamInfo(tm, VectorToStringNoSpace(set));

         // Keep track of elite team history and only save checkpoints
         // when we have a new champion for this phase
         if (GetState("phase") == _TRAIN_PHASE &&
             GetParam<int>("write_checkpoints") &&
             elite_team_id_history_.find(tm->id_) ==
                 elite_team_id_history_.end()) {
            elite_team_id_history_.insert(tm->id_);
...
void TPG::InitTeams() {
   ...
   for (int t = 0; t < GetParam<int>("n_root_gen"); t++) {
   ...
   oss << " eLSz " << num_elite_teams_[GetState("phase")] << endl;
}
...
/******************************************************************************/
void TPG::LogTeamInfo(team* tm, std::string task_set) {
   // vector<int> behaviourSequence;
   set<team*, teamIdComp> visited_teams;
   vector<int> programInstructionCounts, effectiveProgramInstructionCounts;
   tm->policyInstructions(team_map_, visited_teams, programInstructionCounts,
                          effectiveProgramInstructionCounts);
   set<RegisterMachine*, RegisterMachineIdComp> programs;
   set<team*, teamIdComp> teams;
   tm->GetAllNodes(team_map_, teams, programs);

   vector<int> op_counts_single;
   vector<int> op_counts_tally(instruction::NUM_OP);

   vector<int> memory_size_counts(GetParam<int>("max_memory_size") - GetParam<int>("min_memory_size") + 1);

   fill(op_counts_tally.begin(), op_counts_tally.end(), 0);
   for (auto it = programs.begin(); it != programs.end(); it++) {
      // Maps 2 (min_memory_size) -> index 0, etc.
      memory_size_counts[(*it)->private_memory_[MemoryEigen::kMatrixType_]->memory_size_ - GetParam<int>("min_memory_size")]++;
      op_counts_single = (*it)->op_counts_;
      for (size_t i = 0; i < op_counts_single.size(); i++)
         op_counts_tally[i] += op_counts_single[i];
   }

   std::vector<double> fitness_per_task, team_complexity_per_task,
       op_complexity_per_task, flop_complexity_per_task;
   for (int task = 0; task < GetState("n_task"); task++) {
      fitness_per_task.push_back(
          tm->GetMeanOutcome(GetState("phase"), task, GetState("fitMode")));
      team_complexity_per_task.push_back(
          tm->GetMeanOutcome(GetState("phase"), task, VISITED_TEAMS_IDX));
      op_complexity_per_task.push_back(
          tm->GetMeanOutcome(GetState("phase"), task, INSTRUCTIONS_IDX));
      flop_complexity_per_task.push_back(
          tm->GetMeanOutcome(GetState("phase"), task, FLOPS_IDX));
   }
   SelectionMetricsBuilder builder;
   builder.with_generation(GetState("t_current"))
       .with_phase(GetState("phase"))
       .with_task_set(task_set)
       .with_fitness_per_task(fitness_per_task)
       .with_team_complexity_per_task(team_complexity_per_task)
       .with_op_complexity_per_task(op_complexity_per_task)
       .with_flop_complexity_per_task(flop_complexity_per_task)
       .with_team_id(tm->id_)
       .with_team_size(tm->size())
       .with_age(GetState("t_current") - tm->gtime_)
       .with_n_outcomes(tm->numOutcomes(GetState("phase"), GetState("task")))
       .with_fitness_value_for_selection(tm->fit_)
       .with_team_count(teams.size())
       .with_program_count(programs.size())
       .with_total_program_instructions(accumulate(
           programInstructionCounts.begin(), programInstructionCounts.end(), 0))
       .with_total_effective_program_instructions(
           accumulate(effectiveProgramInstructionCounts.begin(),
                      effectiveProgramInstructionCounts.end(), 0))
       .with_operations_use(op_counts_tally)
       .with_memory_size_counts(memory_size_counts);
   SelectionMetrics metrics = builder.build();
   EventDispatcher<SelectionMetrics>::instance().notify(EventType::SELECTION,
                                                        metrics);
}
...
void TPG::FinalizeStepData(EvalData& eval_data) {
   ...
   eval_data.stats_double[FLOPS_IDX] /= eval_data.n_prediction;
