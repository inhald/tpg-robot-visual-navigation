...
RegisterMachine::RegisterMachine(
   ...
   auto memory_size = std::any_cast<int>(params["memory_size"]);

   // If memory_size will be evolved, initialize with a random size
   if (std::any_cast<double>(params["p_memory_size"]) > 0.0) {
   ...
   SetupMemory(state, std::any_cast<int>(params["n_memories"]), memory_size);
}
...
RegisterMachine::RegisterMachine(RegisterMachine &rm) {
   ...
   op_counts_.resize(instruction::NUM_OP);
   effective_flops_ = rm.effective_flops_;
...
RegisterMachine::RegisterMachine(
   ...
   effective_flops_ = atoi(outcomeFields[f++].c_str());   
   ...
   for (size_t t = 0; t < instructions_.size(); t++) {
      ...
      for (auto istr : instructions_) {
         if (memories_effective[istr->GetOutType()]
            ...
            op_counts_[istr->op_]++;
            effective_flops_ += istr->ComputeCost(
                private_memory_[MemoryEigen::kMatrixType_]->memory_size_);
            for (int in = 0; in < 2; in++) {
               if (istr->IsMemoryRef(in)) {
                  memories_effective[istr->GetInType(in)]
                                    [istr->GetInIdx(in) % n_memories] = true;
...
void RegisterMachine::Run(EvalData& eval_data) {
   ...
   for (auto istr : (eval_data.skip_introns ? instructions_effective_ : instructions_)) {
         ...
         if (istr->GetInType(in) != -1) {
            if (istr->IsMemoryRef(in)) {
               ...
               istr->GetInMem(in)->read_time_[istr->GetInIdxE(in)] =
                   eval_data.timestep + (eval_data.team_path.size() / MAX_GRAPH_DEPTH);
