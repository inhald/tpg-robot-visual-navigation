...
class instruction {
  public:
   ...
   static const int COPY_SCALARS_TO_VECTOR_OP_ = 72;
   ...
   inline void ExecuteCopyScalarsToVectorOp(bool dbg) {
      int n_scalar_to_copy = min((int)(out_->memory_size_), memIndices_);
      for (int i = 0; i < n_scalar_to_copy; i++) {
         //TODO(skelly): alwas copying to v1 (vector action)
         out_->working_memory_[1](i, 0) = in1_->working_memory_[i](0, 0);
      }
      if (dbg) {}
   }
   ...
   inline void ExecuteObsBuffSliceOp(bool dbg) {
      ...
      if (dbg) {
         // TODO(skelly): improve debug format
         cerr << "from ";
         for (size_t i = 0; i < in1_->working_memory_.size(); i++) {
   ...
   // Return rough number of FLOPs
   size_t ComputeCost(const size_t& memory_size) {
      switch (op_) {
         case SCALAR_SUM_OP_:
            return 1;
         ...
         case MATRIX_PRODUCT_OP_:
            ...
            return 2 * std::pow(memory_size, 3);
         ...
         case SCALAR_MATRIX_PRODUCT_OP_:
            return std::pow(memory_size, 2);
         case MATRIX_VECTOR_PRODUCT_OP_:
            return 2 * std::pow(memory_size, 2);
         ...
         case MATRIX_ST_DEV_OP_:
            // return std::pow(memory_size, 2); 
            return 4 * std::pow(memory_size, 2);
         ...
         case SCALAR_MATRIX_ASSIGN_OP_:
            return 1;
         case COPY_SCALARS_TO_VECTOR_OP_:
            return memory_size;
         case OBS_BUFF_SLICE_OP_:
            return memory_size;
         default:
            std::cerr << "No complexity for op " << op_ << std::endl;
            return 1;
