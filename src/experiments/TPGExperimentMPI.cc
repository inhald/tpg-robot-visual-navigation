...
int main(int argc, char** argv) {
   ...
   //set MPI partition
   std::string partition = initIgnPartition(world.rank());
   ...
   int max_memory_size = tpg.GetParam<int>("max_memory_size");

   if (tpg.GetParam<int>("replay") == 0) {
      // Initialize logger classes
      SelectionStorage::instance().init(seed_tpg, pid, tpg.GetState("n_task"), max_memory_size);
      SelectionLogger selectionLogger;
      selectionLogger.init();
      TimingStorage::instance().init(seed_tpg, pid, tpg.GetState("n_task"), max_memory_size);
      TimingLogger timingLogger;
      timingLogger.init();
      ReplacementStorage::instance().init(seed_tpg, pid, tpg.GetState("n_task"), max_memory_size);
      ReplacementLogger replacementLogger;
      replacementLogger.init();
      RemovalStorage::instance().init(seed_tpg, pid, tpg.GetState("n_task"), max_memory_size);
