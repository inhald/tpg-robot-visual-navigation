...
// keyboard callback
inline void keyboard(GLFWwindow* window, int key, int scancode, int act,
                     int mods) {
...
inline void InitVisualization(mjModel* task_m, mjData* task_d) {
   ...
   if (!headless) {
    ...
    //TODO(skelly): headless currently disabled
    // linking OSMesa results in "ERROR: gladLoadGL error"
    ...
    //   int height = 900;
    //   osmesa_buffer =
    //       (unsigned char*)malloc(width * height * 4 * sizeof(GLubyte));
    //   if (!osmesa_buffer) {
    //      mju_error("OSMesa buffer allocation failed");
    //   }

    //   // Bind the buffer to the context and make it current
    //   if (!OSMesaMakeCurrent(osmesa_ctx, osmesa_buffer, GL_UNSIGNED_BYTE, width,
    //                          height)) {
...
inline void StepVisualization(TPG& tpg) {
   if (!headless) {
      ...
      if (con.currentBuffer != mjFB_OFFSCREEN) {
         printf(
             "Warning: offscreen rendering not supported, using default/window "
             "framebuffer\n");
      }
      ...
      unsigned char* rgb =
          (unsigned char*)malloc(3 * viewport.width * viewport.height);
      if (!rgb) {
...
inline void EvalMuJoco(TPG& tpg, EvalData& eval_data) {
   ...
   TaskEnv::Results r;
   // cerr << "x_a,y_a,x_t,y_t" << endl;  // TODO(skelly): integrate
   while (!task->terminal()) {

      // ant TODO(skelly): integrate
      // cerr << task->GetObsVec(eval_data.partially_observable)[4];
      // cerr << "," << task->GetObsVec(eval_data.partially_observable)[5];
      // cerr << "," << task->GetObsVec(eval_data.partially_observable)[2];
      // cerr << "," << task->GetObsVec(eval_data.partially_observable)[3] << endl;
      
      eval_data.timestep = task->step_;
      tpg.GetAction(eval_data);
      // auto ctrl = WrapVectorActionMuJoco(eval_data);
      auto ctrl = WrapVectorActionTanh(eval_data);  // TODO(skelly): ablate
      r = task->sim_step(ctrl);
      eval_data.stats_double[REWARD1_IDX] += r.r1;
      eval_data.AccumulateStepData();
      eval_data.obs->Set(task->GetObsVec(eval_data.partially_observable));
      MaybeAnimateStep(tpg);
   }
   eval_data.stats_double[REWARD2_IDX] = r.r2;  // Solved?
