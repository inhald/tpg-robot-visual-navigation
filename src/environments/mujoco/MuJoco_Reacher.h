#ifndef MuJoco_Reacher_h
#define MuJoco_Reacher_h

#include <MuJocoEnv.h>
#include <misc.h>

class MuJoco_Reacher : public MuJocoEnv {
  private:
   int id_fingertip_;
   int id_target_;
   double solved_dist_threshold_ = 0.03;
  ...
  public:
   ...
   MuJoco_Reacher(std::unordered_map<std::string, std::any>& params) {
      eval_type_ = "MuJoco";
   ...
   bool terminal() { return step_ >= max_step_ || (solved_ && terminate_when_solved_); }

   Results sim_step(std::vector<double>& action) {

      // TODO(skelly): integrate
      // cerr << d_->xpos[id_fingertip_ * 3 + 0];
      // cerr << "," << d_->xpos[id_fingertip_ * 3 + 1];
      // cerr << "," << d_->xpos[id_target_ * 3 + 0];
      // cerr << "," << d_->xpos[id_target_ * 3 + 1] << endl;

      auto dist_diff = get_dist();
      auto dist = std::sqrt(std::pow(dist_diff[0], 2) + std::pow(dist_diff[1], 2));
      // Check solved and never reset solved state during an episode
