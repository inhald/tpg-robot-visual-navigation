...
#include "../src/environments/mujoco/MuJoco_Half_Cheetah.h"
#include <random>
#include <TPG.h>
...
TEST_CASE("MuJoco_Half_Cheetah Initialization", "[init]") {
    std::unordered_map<std::string, std::any> params = createDefaultParams();
    INFO("Initializing Environment");
    MuJoco_Half_Cheetah half_cheetah(params);
...
TEST_CASE("MuJoco_Half_Cheetah Terminal Condition", "[terminal]") {
    std::unordered_map<std::string, std::any> params = createDefaultParams();
    MuJoco_Half_Cheetah half_cheetah(params);
...
TEST_CASE("MuJoco_Half_Cheetah Control Cost", "[control_cost]") {
    std::unordered_map<std::string, std::any> params = createDefaultParams();
    MuJoco_Half_Cheetah half_cheetah(params);
...
TEST_CASE("MuJoco_Half_Cheetah Simulation Step", "[sim_step]") {
    std::unordered_map<std::string, std::any> params = createDefaultParams();
    MuJoco_Half_Cheetah half_cheetah(params);
...
// TEST_CASE("MuJoco_Half_Cheetah Reset Function", "[reset]") {
//     std::unordered_map<std::string, std::any> params = createDefaultParams();
//     MuJoco_Half_Cheetah half_cheetah(params);
