...
#include "../src/environments/mujoco/MuJoco_Humanoid_Standup.h"
#include <random>
#include <TPG.h>
...
TEST_CASE("MuJoco_Humanoid_Standup Initialization", "[init]") {
    std::unordered_map<std::string, std::any> params = createDefaultParams();
    INFO("Initializing Environment");
    MuJoco_Humanoid_Standup humanoid(params);
...
TEST_CASE("MuJoco_Humanoid_Standup Terminal Condition", "[terminal]") {
    std::unordered_map<std::string, std::any> params = createDefaultParams();
    MuJoco_Humanoid_Standup humanoid(params);
...
TEST_CASE("MuJoco_Humanoid_Standup Simulation Step", "[sim_step]") {
    std::unordered_map<std::string, std::any> params = createDefaultParams();
    MuJoco_Humanoid_Standup humanoid(params);
...
TEST_CASE("MuJoco_Humanoid_Standup Get Observation", "[get_obs]") {
    std::unordered_map<std::string, std::any> params = createDefaultParams();
    MuJoco_Humanoid_Standup humanoid(params);
...
// TEST_CASE("MuJoco_Humanoid_Standup Reset Function", "[reset]") {
//     std::unordered_map<std::string, std::any> params = createDefaultParams();
//     MuJoco_Humanoid_Standup humanoid(params);
